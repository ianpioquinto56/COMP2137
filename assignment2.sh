...
TARGET_NET_INTERFACE="eth0" # Confirmed from image file
...
echo "---"
echo "## 🚀 Starting assignment2.sh Configuration Script"
echo "---"

# --- 1. Netplan Configuration (Set 192.168.16.21/24 and Remove all others) ---
echo "### 🌐 Netplan Configuration for $TARGET_NET_INTERFACE"

# Explicitly remove unwanted IPs (192.168.16.241 and 192.168.16.242) if they are currently active.
...
for IP_TO_REMOVE in "${UNWANTED_IPS[@]}"; do
    if ip a show dev "$TARGET_NET_INTERFACE" | grep -q "$IP_TO_REMOVE"; then
        ...
        if [[ $? -eq 0 ]]; then
            ...
            log $? "Failed to remove active unwanted IP $IP_TO_REMOVE. (May not be present or error occurred)."
        fi
    else
...
# The most idempotent way to ensure the IP is exactly 192.168.16.21/24
# is to overwrite the relevant netplan config.
NETPLAN_FILE_MODIFIED=0
if grep -E "^\s*addresses:\s*\[$TARGET_NET_IP/$TARGET_NET_CIDR\]" "$NETPLAN_CONFIG" >/dev/null 2>&1; then
    log 0 "Netplan configuration file already defines $TARGET_NET_IP. No file modification needed."
else
    # Overwrite the netplan configuration for the target interface
    # This structure sets the static IP and removes any secondary IPs from the file.
...
EOF
    if [[ $? -eq 0 ]]; then
        log 0 "Updated $NETPLAN_CONFIG with static IP $TARGET_NET_IP. Secondary IPs removed from config."
...
# Apply the netplan configuration only if the file was modified OR if the primary IP is not active.
# We always apply if we modified the file, OR if the current primary IP doesn't match the desired one.
...
if ! grep -q "^$NEW_HOSTS_ENTRY" "$HOSTS_FILE"; then
    ...
    log 0 "Correct $TARGET_HOSTNAME entry is present. (Removal check inconclusive but likely okay)"
fi

echo "---"

# --- 3. Install and Ensure Software is Running ---
echo "### 📦 Software Installation and Service Status"
...
for USER_ENTRY in "${USERS[@]}"; do
    ...
    # C. Generate SSH Keys (RSA and ED25519)
    for KEY_TYPE in rsa ed25519; do
        ...
        if [[ -n "$GENERATED_KEY" ]] && ! grep -qF "$GENERATED_KEY" "$AUTH_KEYS"; then
            ...
            log 0 "Added generated $KEY_TYPE public key to $AUTH_KEYS."
        else
    ...
    # E. Final permissions for keys
    chown "$USERNAME:$USERNAME" "$AUTH_KEYS" 2>/dev/null
    chmod 600 "$AUTH_KEYS" 2>/dev/null
    log 0 "Set final permissions for $AUTH_KEYS and keys for $USERNAME."
